...
amrc_t amfd_read(const int fd, void* buf, const uint32_t olen) // Returns 0 on success
{
	...
	while (len > 0) {
		ret = read(fd, b, len);
		if (ret < 0) {
			AMLOG_DEFUALT("Error reading, errno %d\n", errno);
			return AMRC_ERROR;
		}
		if (ret == 0) {
			AMLOG_DEFUALT("Could not finish reading. Read %u/%u bytes\n", olen - len, olen);
			return AMRC_ERROR;
		}
		len -= ret;
		b += ret;
...
void amfile_write(const char* filename, const void* data, const uint32_t length)
{
	...
	if (fd < 0) {
		AMLOG_DEFUALT("Failed to open file '%s'\n", filename);
		return;
	}
	...
	close(fd);
	if (rc) {
		AMLOG_DEFUALT("Failed to write to file file '%s'\n", filename);
	}
}
...
void* amfile_read(const char* filename, uint32_t* length)
{
	...
	if (file_size == -1) {
		AMLOG_DEFUALT("Failed to obtain size of '%s'\n", filename);
		return NULL;
	}

	rc = lseek(fd, 0, SEEK_SET);
	if (file_size == -1) {
		AMLOG_DEFUALT("Failed to jump to start of '%s'\n", filename);
		return NULL;
	}

	buf = malloc(file_size);
	if (buf == NULL) {
		AMLOG_DEFUALT("Failed to obtain memory of %ld bytes\n", file_size);
		return NULL;
	}
...
amrc_t skt_listen(const amskt_addr* _addr, amskt_t* fd)
{
	...
	if (_addr->s.sa_family != AF_INET && _addr->s.sa_family != AF_INET6) {
		AMLOG_DEFUALT("Address family %u not supported\n", _addr->s.sa_family);
		return AMRC_ERROR;
	}
	...
	if (skt == -1) {
		AMLOG_DEFUALT("Failed to obtain a socket (error %d)\n", errno);
		return AMRC_ERROR;
	}

	rc = setsockopt(skt, SOL_SOCKET, SO_REUSEADDR, &enable, sizeof(enable));
	if (rc != 0)
		AMLOG_DEFUALT("Failed to set reuse address option on server socket (error %d)\n",	errno);

	rc = setsockopt(skt, SOL_SOCKET, SO_REUSEPORT, &enable, sizeof(enable));
	if (rc != 0)
		AMLOG_DEFUALT("Failed to set reuse port option on server socket (error %d)\n", errno);
	...
	if (rc != 0) {
		AMLOG_DEFUALT("Error binding to ANY:%u (error %d)\n", port, errno);
		goto close;
	}

	rc = listen(skt, 5);
	if (rc != 0) {
		AMLOG_DEFUALT("Can't listen on server socket (error %d)\n", errno);
		goto close;
	}
...
amrc_t skt_connect(const amskt_addr* addr, amskt_t* fd)
{
	...
	if (_fd < 0) {
		AMLOG_DEFUALT("Failed to allocate socket (error %d)\n", errno);
		return AMRC_ERROR;
	}

	if (connect(_fd, &addr->s, sizeof(addr->s)) != 0) {
		AMLOG_DEFUALT("Connect failed to remote socket\n");
		return AMRC_ERROR;
	}

	*fd = _fd;
	AMLOG_DEFUALT("connected to remote address\n");
	return AMRC_SUCCESS;
}
...
amrc_t skt_accept(const amskt_t server, amskt_t* client)
{
	...
	*client = -1;
	...
	if (cskt == -1) {
		AMLOG_DEFUALT("Failed to accept socket (error %d)\n", errno);
		return AMRC_ERROR;
	}
...
{
	if (close(*fd))
		AMLOG_DEFUALT("Failed to disconnect socket %d\n", *fd);
	*fd = -1;
}
...
amrc_t skt_port2addr(const sa_family_t fam, uint16_t port, amskt_addr* addr)
{
	...
	if (fam == AF_INET) {
		if (addr->s4.sin_family != AF_INET && addr->s4.sin_family != AF_UNSPEC) {
			AMLOG_DEFUALT("Re-assigning port to a non-empty address\n");
			goto error;
		}
...
amrc_t skt_str2addr(const char *str, uint16_t port, amskt_addr* addr)
{
	...
	AMLOG_DEFUALT("Failed to resolve '%s' to an address\n", str);
	memset(addr, 0, sizeof(*addr));
	addr->s.sa_family = AF_UNSPEC;
	return AMRC_ERROR;
}
